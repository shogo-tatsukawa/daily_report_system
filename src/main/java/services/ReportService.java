...
public class ReportService extends ServiceBase {
   ...
   // 追加
   /**
    * 指定した複数の従業員が作成した日報データを、指定されたページ数の一覧画面に表示する分取得しReportViewのリストで返却する
    * @param employeeIds 従業員IDのリスト
    * @param page ページ数
    * @return 一覧画面に表示するデータのリスト
    */
   public List<ReportView> getSelectPerPage(List<Integer> employeeIds, int page) {

       List<Report> reports = em.createNamedQuery(JpaConst.Q_REP_GET_ALL_SELECT, Report.class)
               .setParameter(JpaConst.JPQL_PARM_EMPLOYEEIDS, employeeIds)
               .setFirstResult(JpaConst.ROW_PER_PAGE * (page - 1))
               .setMaxResults(JpaConst.ROW_PER_PAGE)
               .getResultList();
       return ReportConverter.toViewList(reports);
   ...
   /**
    * 指定した複数の従業員が作成した日報データの件数を取得し、返却する
    * @param employeeIds 従業員IDのリスト
    * @return 日報データの件数
    */
   public long countAllSelect(List<Integer> employeeIds) {

       long count = (long) em.createNamedQuery(JpaConst.Q_REP_COUNT_ALL_SELECT, Long.class)
               .setParameter(JpaConst.JPQL_PARM_EMPLOYEEIDS, employeeIds)
               .getSingleResult();
